...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(45.f), glm::vec3(1, 1, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1.f, 1.f, 1.3f));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(90.f), glm::vec3(1, 0, 1.3f));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1.5f, 1.f, .7f));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_U) && projectionType == false)
        {
            left += deltaTime;
            projectionMatrix = glm::ortho(left, right, bottom, top, Z_NEAR, Z_FAR);
        }
        if (window->KeyHold(GLFW_KEY_Y) && projectionType == false)
        {
            left -= deltaTime;
            right += deltaTime;
            projectionMatrix = glm::ortho(left, right, bottom, top, Z_NEAR, Z_FAR);
