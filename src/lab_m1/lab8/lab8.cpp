...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    /*lightsPositionArray[0] = lightPosition;
    lightsPositionArray[1] = glm::vec3(1, 1, 1);

    lightsDirectionArray[0] = lightDirection;
    lightsDirectionArray[1] = glm::vec3(0, -1, 0);*/
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    /*GLuint shadersLightPositionArray = glGetUniformLocation(shader->program, "lights_position");
    glUniform3fv(shadersLightPositionArray, 9, lightsPositionArray);*/

    int shaderSpotLight = glGetUniformLocation(shader->program, "switchSpotLight");
    glUniform1i(shaderSpotLight, spotLight);

    float shaderSpotAngle = glGetUniformLocation(shader->program, "spotAngle");
    glUniform1f(shaderSpotAngle, spotAngle);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    {
        if (window->KeyHold(GLFW_KEY_EQUAL)) spotAngle += deltaTime * 10;
        if (window->KeyHold(GLFW_KEY_MINUS)) spotAngle -= deltaTime * 10;
    }
}
...
{
    // Add key press event
    if (key == GLFW_KEY_F) spotLight == 0 ? spotLight = 1 : spotLight = 0;
